...
function renderDowTable(range: RangeAgg, mode: MeasurementMode): string[] {
  ...
  const lines: string[] = [];
  lines.push(
    `${padRight("day of week", dowWidth)}  ${padLeft(kind, valueWidth)}  ${padLeft("cost", 10)}  ${padLeft("share", 6)}`,
  );
  lines.push(
...
class BreakdownComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    const graphDescriptor =
      this.view === "dow" ? `share of ${metric.kind} by day of week` : `${metric.kind} per day`;
    const summary =
    ...
    if (this.view === "dow") {
      ...
      if (showSideLegend) {
        ...
        const legendTitle =
          this.view === "model"
            ? "Top models by recent usage:"
            : this.view === "cwd"
              ? "Top directories by recent usage:"
              : "Time-of-day buckets:";
