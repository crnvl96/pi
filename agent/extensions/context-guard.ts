...
export default function (pi: ExtensionAPI): void {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    const absolutePath = resolve(ctx.cwd, rawPath.startsWith("@") ? rawPath.slice(1) : rawPath);
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (usesUnboundedRg(cmd)) {
      ...
      ctx.ui.notify(`[context-guard] bash: appended | head -${cfg.rgHeadLimit} to rg`, "info");
    }
  });
}
