...
const COMPACT_THRESHOLD_TOKENS = 217_000;
...
export default function (pi: ExtensionAPI) {
  pi.on("session_before_compact", async (event, ctx) => {
    ...
    // Use a fast model for summarization (cheaper/faster than most conversation models)
    const model = ctx.modelRegistry.find("openai-codex", "gpt-5.4-mini");
    if (!model) {
      ctx.ui.notify(`Could not find fast model, using default compaction`, "warning");
      return;
    }
    ...
    const summarySystemPrompt = `You are a conversation summarizer. Create a comprehensive summary of this conversation that captures:
    ...
    const summaryMessages = [
      {
        ...
        content: [
          {
            type: "text" as const,
            text: `${previousContext}<conversation>
${conversationText}
</conversation>`,
          },
        ],
    ...
    try {
      ...
      const response = await complete(
        model,
        { systemPrompt: summarySystemPrompt, messages: summaryMessages },
        {
          apiKey: auth.apiKey,
