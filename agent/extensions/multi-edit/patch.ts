...
function applyHunks(filePath: string, content: string, hunks: Hunk[]): string {
  ...
  for (const hunk of hunks) {
    ...
    result =
      result.slice(0, match.pos) + hunk.newBlock + result.slice(match.pos + match.matchLength);
  ...
  for (const op of ops) {
    ...
    switch (op.kind) {
      case "add": {
        ...
        const oldText =
          collectDiff && (await workspace.exists(abs)) ? await workspace.readText(abs) : "";
        const newText = ensureTrailingNewline(op.contents);
        await workspace.writeText(abs, newText);
        results.push(
          buildOpResult(op.path, `Added file ${op.path}.`, oldText, newText, collectDiff),
        );
        break;
      }
      ...
      case "update": {
        ...
        results.push(
          buildOpResult(op.path, `Updated ${op.path}.`, sourceText, updated, collectDiff),
        );
        break;
      }
    }
